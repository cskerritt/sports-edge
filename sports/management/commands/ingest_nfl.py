...
class Command(BaseCommand):
    help = "Ingest NFL teams, schedule, scores, and injuries"
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stderr.write(self.style.ERROR(f"NFL ingest failed: {exc}"))
            logger.exception("NFL ingest unhandled exception")
            raise SystemExit(1)

        self.stdout.write(self.style.SUCCESS(f"NFL ingest complete: {result}"))
