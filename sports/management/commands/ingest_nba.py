...
class Command(BaseCommand):
    help = "Ingest NBA teams, schedule, scores, and injuries"
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stderr.write(self.style.ERROR(f"NBA ingest failed: {exc}"))
            logger.exception("NBA ingest unhandled exception")
            raise SystemExit(1)

        self.stdout.write(self.style.SUCCESS(f"NBA ingest complete: {result}"))
