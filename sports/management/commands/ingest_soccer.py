...
class Command(BaseCommand):
    help = "Ingest Soccer teams, schedule, scores, and injuries"
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stderr.write(self.style.ERROR(f"Soccer ingest failed: {exc}"))
            logger.exception("Soccer ingest unhandled exception")
            raise SystemExit(1)

        self.stdout.write(self.style.SUCCESS(f"Soccer ingest complete: {result}"))
