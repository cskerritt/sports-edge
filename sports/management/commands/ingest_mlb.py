...
class Command(BaseCommand):
    help = "Ingest MLB teams, schedule, scores, and injuries"
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stderr.write(self.style.ERROR(f"MLB ingest failed: {exc}"))
            logger.exception("MLB ingest unhandled exception")
            raise SystemExit(1)

        self.stdout.write(self.style.SUCCESS(f"MLB ingest complete: {result}"))
